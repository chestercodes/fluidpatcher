...
#    apt_pkg_install "git"  # use curl or wget instead
    ...
    rm -rf fluidpatcher
    git clone https://github.com/chestercodes/fluidpatcher &>> $logfile
	cd fluidpatcher
...
if [[ $audiosetup =~ ^(1|2|3)$ ]]; then
	...
	sudo sed -i "/dtoverlay=hifiberry-dacplusadcpro/d" $CONFIG
fi
if [[ $audiosetup == "1" ]]; then
    echo "Activating DAC sound..."
	echo "dtoverlay=hifiberry-dacplusadcpro" | sudo tee -a $CONFIG &>> $logfile
    ASK_TO_REBOOT=true
